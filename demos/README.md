...
| `bladed_iris_star` | 633 nm coherent Ø0.6 beam floods a **6-blade** iris whose true HEXAGONAL opening (Ø0.30 mm inscribed, air-filled polygon plug) clips the wavefront, screen 300 mm on | N-blade aperture diffraction STAR (engine3 §11 / P8): the polygon's 6 straight edges diffract the coherent beam into the classic **6-spike star** (even N → N spikes) in place of `airy_singleslit`'s circular Airy rings. 6-fold azimuthal symmetry asserted quantitatively — the m=6 Fourier component of the detector image dominates (m6/AC ≈ **0.84** vs a circular stop's ≈ 0.07 in the same bin, `test_bladed_iris_star.py`) | passes the Ø0.6 beam clipped to the hexagon |
...
### Samples, cells & scattering instruments (samples-instruments round)

Seven benches exercising the sample-cell / lamp / image-source primitives,
the `sample=`/`instrument=`/`image=` body properties, the samples
scattering registry, and the `--conical`/`--ring-profile`/`--reference-case`
post products. Each carries a **bespoke physics gate** in
`scripts/run_demo_equivalence.py` (beside `gate_fizeau`) that runs
**in addition** to the placement + 3-seed-power baseline oracles. Where the
task's intended cell/particle exceeds the engine's tractable regime the
demo uses a documented, physics-preserving substitute (see the notes column
and `UXNOTES.md` 2026-07-23).

| Demo | System | What it shows · gate | Engine · detected (quick preset) |
|---|---|---|---|
| `conical_refraction` | Collimated incoherent 1064 nm pencil launched along a KTP **optic axis** (`crystal_axis`/`crystal_axis2` from the biaxial registry), 20 mm slab, `--conical --conical-fan 24`, detector 3 mm past the exit | Internal conical refraction (Hamilton's cone): the slab exit freezes a hollow **Poggendorff ring**. **Gate:** radial-mean ring radius = 0.5·t·tan(A) within 10% AND radially tight (std_r < 0.35·mean_r) — measured **0.283 mm vs 0.280 mm (1.1%), std/mean 0.14** | **Python** (biaxial unported) · Screen **0.861 mW**; closes |
| `forward_scatter_diffraction_sizer` | Collimated 633 nm → bare `sample_region` of 10 µm glass beads (`sample=glass_beads_10um_water` continuum) → BK7 PCX Fourier lens → ring detector at the back focal plane; `--ring-profile n=24:rmin=0.05:rmax=8` | ISO-13320 laser diffraction: the forward-diffraction lobe maps angle→radius (r=f·θ). **Gate:** the scattered lobe's first-null (half-power outer edge) sits at r=f·1.22λ/d=**3.49 mm** within ±1 log-ring (measured **3.84 mm**), ring integration closes. *(Deviation: a glass `flow_cell` TIR-traps wide-angle scatter → C-engine divergence — bare region instead, see UXNOTES)* | **C** · RingDet **4.38 mW**; closes |
| `goniometer_bath` | Incoherent 632.8 nm → `sample_region` of a colloidal silica gel (`sample=silica_gel_fractal` continuum) → three detectors on a goniometer arc (θ=60/90/120°) driven by anchored **pose expressions** over a `theta_i`/`R` variable set | Static light scattering: the fractal form factor I(q)~q^−df. **Gate:** log-log I(q) slope = −df = **−2.10** (target −2.1 ±0.3, registry df=2.1). *(Deviation: the intended decalin vat/vial glass TIR-traps wide-angle scatter → C divergence — bare region instead, see UXNOTES)* | **C** · Det60/90/120 **~1e-4 mW** each; closes |
| `uvvis_spectrometer` | Broadband lamp (`tungsten_halogen`, visible band) → square cuvette of **KMnO₄** dye (`dye_solution_kmno4`, 10 mm path) → slit → crossed Czerny-Turner → detector carrying `instrument=tcd1304_array` | UV-Vis absorbance: **two-run gate** re-runs a BLANK (`--suppress-body` the dye liquid) and posts the sample with `--reference-case` to form A(λ)=−log₁₀(I/I₀). **Gate:** A peaks at **525 ±15 nm** with peak A within 30% of Beer-Lambert (α=769/m, 10 mm → **A=3.34**) | **C** · Array **0.324 mW** dispersed; closes |
| `imaging_bench` | Backlit USAF-1951-style target (`source_image`, `image=usaf_style_target`, `image_cone_deg=8`) relayed **1:1 (2f:2f)** by a BK7 DCX singlet onto a detector; `--save-fields` | Finite-conjugate imaging: `render_image_traced` produces the traced end-to-end image (auto, because the source carries `image=`). **Gate:** NCC(traced, USAF target) at matched resolution, any dihedral orientation (detector-basis handedness) > 0.5 — measured **0.82** | **C** · RelayDet **16.6 mW**; closes |
| `colloidal_crystal` | Broadband beam → 45° plate BS → `sample_region` carrying the FCC (111) paracrystal (`sample=colloidal_fcc_continuum`, S(q) structure factor) → forward + backscatter (BS +y return arm) detectors | Structural-colour Bragg backscatter near λ_Bragg=2·n·d₁₁₁=**538 nm**. **Gate:** the normalised reflectance R(λ)=back/fwd is wavelength-selective (blue/red > 1.8; measured **1.93**). *(Deviation: the explicit FCC lattice needs ~1e12 sites/136 TiB in a macroscopic cell — continuum paracrystal S(q) instead; the sharp coherent single-bin Bragg peak is not resolved, see UXNOTES)* | **C** · Forward **0.80 mW**, Backscatter **0.036 mW**; closes |
| `dls_goniometer` | Coherent 632.8 nm → small water `sample_region` of a **few sparse 3 µm PSL** spheres (`sample=psl_dls_3um`, explicit) → three small forward detectors; the slow gate runs `run_dls.py` (60 frames) + `dls_correlate.py` | Traced-dynamics dynamic light scattering: a decorrelating coherent speckle sequence → g1(τ) cumulant fit → Γ, D, d_H. **Status:** the builder ships and `run_dls` produces a populated speckle field; a fully-resolved Γ-vs-D·q² gate needs a water-immersion **ambient** (large-angle detectors without TIR) that the extractor hardcodes to air, so `dls_goniometer` ships as a **documented demonstrator**, not a baseline oracle (see UXNOTES). *(100 nm PSL scatters ~9 orders too weakly for a discrete-ray coherent gather; 3 µm sparse spheres are the proven `test_dls.py` recipe)* | **C**/Python · builder + `run_dls` field; not baseline-gated |
