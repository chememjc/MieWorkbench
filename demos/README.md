...
| `insitec_sizer` | Collimated 633 nm → bare `sample_region` of 10 µm glass beads (`sample=glass_beads_10um_water` continuum) → BK7 PCX Fourier lens → ring detector at the back focal plane; `--ring-profile n=24:rmin=0.05:rmax=8` | ISO-13320 laser diffraction: the forward-diffraction lobe maps angle→radius (r=f·θ). **Gate:** the scattered lobe's first-null (half-power outer edge) sits at r=f·1.22λ/d=**3.49 mm** within ±1 log-ring (measured **3.84 mm**), ring integration closes. *(Deviation: a glass `flow_cell` TIR-traps wide-angle scatter → C-engine divergence — bare region instead, see UXNOTES)* | **C** · RingDet **4.38 mW**; closes |
