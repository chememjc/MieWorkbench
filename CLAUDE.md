...
scenes · `env/` GUI venv (gitignored) · `var/` workspaces/caches
(gitignored).
...
- **Primitives** (`scripts/primitivelib.py` + `primitives/*.FCStd` +
  `.meta.json`): 52 catalog elements (sources/detectors/lenses/
  beamsplitters/filters/polarization/prisms & mirrors/apertures/
  diffusers); geometry params live in a `dim` spreadsheet; edits go
  through **rebuild-on-edit** (`rebuild_primitive` op re-runs the builder —
  constraint expressions can't change topology). Hand-authored primitives
  with real cell expressions use the normal `set_spreadsheet` path instead.
  Conventions: aperture params are `diameter`/`width` (LEGACY_ALIASES in
  `read_params` migrates old radius/half sheets ×2), `round_flag` picks
  circular vs rectangular on plates and sources, `derived_props` lets a
  builder-owned body property track a sheet param through rebuilds (iris
  `blackness` → `absorbance`), and face lookups must use the SIGN-AWARE
  `_find_face_by_signed_normal` (the abs-dot helper can't tell front from
  back caps). BS ratio / ND density / diffuser grit are swapped via the
  coating/filter/diffuser property rows (`bs_XXYY_vis_45`, `nd_odXX`,
  `@dg_XXX`), generated by `scripts/tools/gen_registry_rows.py`.
