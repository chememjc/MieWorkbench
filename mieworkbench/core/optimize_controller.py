"""OptimizeController - Qt-only wrapper around QProcess for driving
scripts/optimize.py (the merit-function optimizer) from the GUI.

The mirror image of core/runner.py's RunController, with two deliberate
differences:

* the program is the OPTICS env python (common.OPTICS_PYTHON — optimize.py
  needs scipy/nevergrad and fast_eval's imports), not system python3;
* build_args() walks cli_specs.build_parser("optimize") instead of
  "pipeline", so the pane's config dict and the real CLI can never drift.

Protocol: MIEWB_PROGRESS=1 is set so every evaluation emits an
'@MIEWB {"stage":"optimize","frac":evals/budget,...}' line (plus per-eval
extras: eval/budget/merit/best/params/best_params — see optimize.py's
progress_emit call); those become progress(dict) signals the OptimizePane
feeds its convergence plot from. Everything else becomes line(str) for
the console pane. Cancellation mirrors RunController: terminate(), then
...
OPTIMIZE_SCRIPT = REPO_DIR / "scripts" / "optimize.py"

STOP_GRACE_MS = 5000


class OptimizeController(QObject):
    """Owns at most one scripts/optimize.py QProcess at a time."""
    ...
    def __init__(self, settings=None, parent=None, python=None,
        ...
        self.script = str(script or OPTIMIZE_SCRIPT)
        self._proc = None
    ...
    def start(self, model_path, extra_args=None, extra_env=None):
        """Launch optimize.py --model <model_path> <extra_args>. Returns
        True if launched, False if an optimization was already in
        ...
        if not self._proc.waitForStarted(5000):
            self.error.emit("failed to start optimize.py")
            self._proc = None
            return False
        self.started.emit()
        return True
    ...
    def build_args(config):
        """{dest: value} (as produced by OptimizePane.config()) ->
        optimize.py argv, using cli_specs.build_parser("optimize") as the
        single source of truth for option strings / defaults / append vs
        scalar handling. Values equal to the parser default are skipped;
        --model is passed separately by start()."""
