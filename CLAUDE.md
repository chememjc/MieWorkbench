...
- **Primitives** (`scripts/primitivelib.py` + `primitives/*.FCStd` +
  `.meta.json`): geometry params live in a `dim` spreadsheet; edits go
...
- optic extras (stackable): `coating` (whole-body or `Face3=MgF2;...`),
  `roughness`, `filter`, `polarizer` + `polarizer_axis`, `crystal_axis`,
  `grating` (`Face2=600:v:orders=-1..1` or `Face2=@vbg_1800`),
  `surface_override` (asphere, verified <1 µm), `mirror`, `absorbance`
